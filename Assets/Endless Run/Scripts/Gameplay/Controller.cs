...
//          animationManager.animationState = animationManager.Roll;
    ...
    private void TouchInput()
    {
        ...
        if (Input.GetMouseButton(0) && checkSideCollision == false)
        {
            if (activeInput)
            {
                ...
                if ((Input.mousePosition.y - currentPos.y) > 40)
                {
                    directInput = DirectionInput.Up;
                    activeInput = false;
                }
    ...
    private void DirectionAngleInput()
    {
        ...
        if (Input.GetMouseButton(0) && checkSideCollision == false)
        {
            if (activeInput)
            {
                ...
                if ((Input.mousePosition.y - currentPos.y) > 20)
                {
    ...
    void OnTriggerEnter(Collider collider)
    {
        ...
        {
            if (collider.GetComponent<Item>().useAbsorb)
            {
                collider.GetComponent<Item>().useAbsorb = false; 
                collider.GetComponent<Item>().StopAllCoroutines();
            }
            collider.GetComponent<Item>().ItemGet(); 
        }
        else
